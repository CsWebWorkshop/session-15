    ...
    if($_SERVER["REQUEST_METHOD"] == 'POST') {
        if(isset($_POST['email']) && isset($_POST['pass']) && isset($_POST['rpass'])) {
            ...
            if ($errors == []) {
                ...
                $checkemail = $conn->query("SELECT id FROM user WHERE email='".$_POST['email']."'");
                if($checkemail->num_rows > 0) {
                    $errors[] = 'Email already in use!';
                } else {
                    $result = $conn->query("INSERT INTO `user` (`email`, `password`) VALUES ('".$_POST['email']."', '".password_hash($_POST['pass'], PASSWORD_DEFAULT)."')");
                    if ($result == true) {
